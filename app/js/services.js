...
var storage = window.localStorage,
    agentCode, source = 'YZTZ',
    device = 1;
//请求地址服务
myServices.factory('URLService', function ($routeParams) {
    if ($routeParams.agent_code && $routeParams.agent_code != '') { //在这里就提取agent_code并设置到localstorage，将变量都赋值。在app.js和controller.js中设置都没用，因为这边是最先运行的。
        agentCode = $routeParams.agent_code;
        storage.setItem('agentCode', $routeParams.agent_code);
        if (agentCode == 'QHSSJ') {
            source = 'APPSTORE';
            device = 3;
        }
    ...
    var urls = {
        ...
        //衫得支付
        sendPay: '/pay/payGateway/sandPayH5Pay.json',
        //支付H5
        alipayH5Pay: '/pay/payGateway/alipayH5Pay.json',
        //支付接口列表
...
//用户服务
myServices.factory('UserService', function ($http, $httpParamSerializerJQLike, URLService) {
    return {
        ...
        getSimBalance: function () {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('getSimBalance'),
                data: $httpParamSerializerJQLike(data)
            });
        },
        //获取银行信息
        ...
        outerSignAgreement: function (commodityNo) {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('outerSignAgreement'),
                data: $httpParamSerializerJQLike(data)
            });
        },
        //发送手机解绑短信验证码
...
myServices.factory('PayService', function ($http, $httpParamSerializerJQLike, URLService) {
    return {
        ...
        alipayH5Pay: function (money) {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('alipayH5Pay'),
                data: $httpParamSerializerJQLike(data)
            });
        },
        //联动支付
...
myServices.factory('NoticeService', function ($http, $httpParamSerializerJQLike, URLService) {
    return {
        ...
        getNewsList: function (page, pageSize, time) {
            ...
            return $http({
                method: 'GET',
                url: URLService.getURL('getNewsList'),
                params: params
            });
        },
        //改版公告
...
myServices.factory('TradeService', function ($http, $httpParamSerializerJQLike, URLService) {
    return {
        ...
        getTradeResult: function (page, pageSize, type) {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('getSettleStrategy'),
                data: $httpParamSerializerJQLike(data)
            });
        },
        //平仓-发起
        getCloseFutures: function (ids, type) {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('closeFutures'),
                data: $httpParamSerializerJQLike(data)
            });
        },
        //外盘-设置止盈止损线
        setQuitGainLoss: function (id, type, quitLoss, quitGain) {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('setQuitGainLoss'),
                data: $httpParamSerializerJQLike(data)
            });
        },
        //风控参数
        ...
        createFuturesStrategy: function (data) {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('createFuturesStrategy'),
                data: $httpParamSerializerJQLike(data)
            });
        },
        //获取单条期货策略信息
        getStrategyInfo: function (id) {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('getStrategyInfo'),
                data: $httpParamSerializerJQLike(data)
            });
        }
    };
...
myServices.factory('SystemService', function ($http, URLService) {
    ...
    return {
        ...
        isHoliday: function (time) {
            ...
            var holiday = holidays[currencyType],
                now = time || new Date(systemTime);
        ...
        getRealPeriod: function (commNo, time, type) {
            ...
            if (X.isArray(period[0][0])) {
                while (true) {
                    ...
                    var bbt = prevTime[0][0].replace(":", "") - 0,
        ...
        isInPeriod: function (commNo, type) {
            ...
            var prevIsHoliday = this.isHoliday(prevDay),
                isHoliday = this.isHoliday();
            var time = X.formatDate(now, 'h:m'),
        ...
        agentIndex: function () {
            ...
            if (agentCode == '' || !agentCode) {
                indexUrl = '/index';
                return indexUrl;
            } else if (agentCode != '' && isApp == 'YES') {
                indexUrl = '/index/' + agentCode + '?source=IOSAPP';
                return indexUrl;
            }
        },
        /**
        ...
        getTipsForNextTime: function (commNo) {
            ...
            var len = _period.length,
                i, nextTime;
            for (i = 0; i < len; i++) {
                var timePeriod = _period[i],
                    bt = timePeriod[0];
                if ("00:00" !== bt && time < bt) {
                    nextTime = bt;
                    break;
        ...
        getCities: function () {
            var cities = [
                ...
                ["23", "251", "巴中市", "635500"],
                ["23", "252", "资阳市", "641300"],
                ["23", "253", "阿坝藏族羌族自治州", "624600"],
                ["23", "254", "甘孜藏族自治州", "626000"],
                ["23", "255", "凉山州", "615000"],
                ["24", "256", "贵阳市", "55000"],
                ["24", "257", "六盘水市", "553000"],
                ["24", "258", "遵义市", "563000"],
                ...
                ["28", "308", "定西市", "743000"],
                ["28", "309", "陇南市", "742100"],
                ["28", "310", "临夏回族自治州", "731100"],
                ["28", "311", "甘南藏族自治州", "747000"],
                ["29", "312", "西宁市", "810000"],
                ["29", "313", "海东地区", "810600"],
                ["29", "314", "海北藏族自治州", "810300"],
                ["29", "315", "黄南藏族自治州", "811300"],
                ["29", "316", "海南藏族自治州", "813000"],
                ["29", "317", "果洛藏族自治州", "814000"],
                ["29", "318", "玉树藏族自治州", "815000"],
                ["29", "319", "海西蒙古族藏族自治州", "817000"],
                ["30", "320", "银川市", "750000"],
                ["30", "321", "石嘴山市", "753000"],
                ["30", "322", "吴忠市", "751100"],
                ["30", "323", "固原市", "756000"],
                ...
                ["31", "327", "吐鲁番地区", "838000"],
                ["31", "328", "哈密地区", "839000"],
                ["31", "329", "昌吉回族自治州", "831100"],
                ["31", "330", "博尔塔拉蒙古自治州", "833400"],
                ["31", "331", "巴音郭楞蒙古自治州", "841000"],
                ["31", "332", "阿克苏地区", "843000"],
                ["31", "333", "克孜勒苏柯尔克孜自治州", "835600"],
                ["31", "334", "喀什地区", "844000"],
                ["31", "335", "和田地区", "848000"],
                ["31", "336", "伊犁哈萨克自治州", "833200"],
                ["31", "337", "塔城地区", "834700"],
                ["31", "338", "阿勒泰地区", "836500"],
                ["31", "339", "石河子市", "832000"],
                ["31", "340", "阿拉尔市", "843300"],
                ["31", "341", "图木舒克市", "843900"],
                ["31", "342", "五家渠市", "831300"],
                ["32", "343", "香港特别行政区", "000000"],
                ["33", "344", "澳门特别行政区", "000000"],
                ["34", "345", "台湾省", "000000"]
            ];
            return cities;
        },
        ...
        beyondTradeTimeTips: function (commNo) {
            var _period = [],
                str = [],
                period = this.getQueryPeriod(commNo, 'trade');
            var i, len, head = '非交易时间不可交易<br><br>【{{COMMNO}}的买入时间】<br>',
                sc;
            var commodity = {
                HSI: '恒指',
                GC: '美黄金',
...
//股票和弹窗服务
myServices.factory('ADBannerService', function ($http, $httpParamSerializerJQLike, URLService) {
    return {
        getADBannerData: function () {
            ...
            return $http({
                method: 'GET',
                url: URLService.getURL('getADBanner'),
                params: params
            });
        }
    };
...
myServices.factory('PacketService', function ($http, $httpParamSerializerJQLike, URLService) {
    return {
        ...
        getActivityInfo: function () {
            ...
            return $http({
                method: 'GET',
                url: URLService.getURL('getActivityInfo'),
                params: params
            })
        },
        //子活动列表
        getActAwardSetInfo: function (activityId) {
            ...
            return $http({
                method: 'GET',
                url: URLService.getURL('getActAwardSetInfo'),
                params: params
            })
        },
        //红包领取
...
//推广赚钱
myServices.factory('ExtensionService', function ($http, $httpParamSerializerJQLike, URLService) {
    return {
        getExtensionInfoData: function () {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('getExtensionInfo'),
                params: params
            });
        },
        getExtensionUserListData: function (page, pageSize) {
            ...
            return $http({
                method: 'POST',
                url: URLService.getURL('getExtensionUserList'),
                params: params
            });
        }
        /*,
                 ...
                 return $http({
                 method: 'GET',
                 url: URLService.getURL('getWechatCode'),
                 params: params
                 });
                 }*/
    };
